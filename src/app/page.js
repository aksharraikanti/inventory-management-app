...
const Home = () => {
  ...
  const categories = ['All', 'Food', 'Electronics', 'Clothing', 'Books']; // Example categories with 'All' option
  ...
  const handleClassify = async () => {
    ...
    const docRef = doc(collection(firestore, 'inventory', user.uid, 'items'), itemName);
    await setDoc(docRef, { name: itemName, quantity: 1, category: itemCategory, classification: result });
  };
        ...
        {user ? (
          ...
          <Box>
            <TextField
              ...
              InputLabelProps={{
                style: { color: 'black' }, // Ensure label is black
              }}
            />
            <TextField
              label="Password"
